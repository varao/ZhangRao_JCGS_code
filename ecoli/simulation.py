...
def experiment(seedIndex, kappa, pi_0, sample_n, T_interval, k, priors, burnin_size):
    ...
    save_list(alpha_list, filename1)
    save_list(beta_list, filename2)
    save_list(lamb1_list, filename4)
    save_list(lamb2_list, filename5)
    plt.hist(alpha_list, color='blue', label='MH', alpha = 0.5)
...
def main():
    n_sample, n_burnin  = int(sys.argv[1]), int(sys.argv[2])
    kappa = 2
    seed=1 
    pi_0 = [.5, .5]
